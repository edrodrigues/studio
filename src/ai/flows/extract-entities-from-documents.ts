...
const extractEntitiesFlow = ai.defineFlow(
  ...
  async input => {
    ...
    // Since the prompt asks for a JSON object, and we set format: 'json',
    // the output should be a valid JSON object. We stringify it to fit the schema.
    return {
      extractedJson: JSON.stringify(output, null, 2),
    };
  }
);
